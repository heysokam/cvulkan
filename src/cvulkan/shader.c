...
cvk_Pure cvk_Shader cvk_shader_create (
  ...
  cvk_Slice code_data = arg->allocator->cpu.duplicate(&arg->allocator->cpu, &(cvk_Slice){
    .ptr= arg->code->ptr, .len= arg->code->len, .itemsize= sizeof(*arg->code->ptr)
  });
  cvk_Shader result = (cvk_Shader){
    .ct       = NULL,
    .code     = (cvk_SpirV){.ptr = (uint32_t*)code_data.ptr, .len = code_data.len},
  };
  result.cfg  = (VkShaderModuleCreateInfo ){
    .sType    = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO,
    .pNext    = NULL,
    .flags    = (VkShaderModuleCreateFlags)0, // Vk.Spec -> Reserved for future use
    .codeSize = result.code.len * sizeof(*result.code.ptr),
...
void cvk_shader_destroy (
  ...
  allocator->cpu.free(/* clang-format off */&allocator->cpu, &(cvk_Slice){
    .ptr= shader->code.ptr, .len= shader->code.len, .itemsize= sizeof(*shader->code.ptr)
  });  // clang-format on
